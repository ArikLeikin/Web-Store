document.addEventListener("DOMContentLoaded", function () {
  ...
  if (total) {
    document.getElementById("total-price").textContent = "Total(USD) $" + total;
  }
});
...
$(document).ready(function () {
  $("form").on("submit", function (event) {
    ...
    if (!validateStreet) {
      ...
      $("#street-error")
        .addClass("error-message")
        .text("Street should contain only letters.");
    } else {
    ...
    if (!validateStreetNumber) {
      ...
      $("#street-number-error")
        .addClass("error-message")
        .text("Street Number should contain only numbers.");
    } else {
    ...
    if (!validateCity) {
      ...
      $("#city-error")
        .addClass("error-message")
        .text("City should contain only letters.");
    } else {
    ...
    if (!validateCreditCard) {
      ...
      $("#credit-card-error")
        .addClass("error-message")
        .text("Credit Card should be 16 digits.");
    } else {
    ...
    if (
      ...
      $("#expiration-date-error")
        .addClass("error-message")
        .text("Expiration date is not valid.");
    } else {
